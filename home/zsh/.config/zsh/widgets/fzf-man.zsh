# $ZCONFDIR/widgets/fzf-man.zsh — fuzzy man page browser. Alt+I.
#
# Actions:
#   Enter     open in nvim with :Man (full navigation, hyperlinks, search)
#   Ctrl+P    open in $PAGER (traditional)
#   Ctrl+T    open tldr page (if tldr/tealdeer installed)
...
_fzf_man_browser() {
  ...
  selected=$(
    ...
    | fzf \
        ...
        --bind='ctrl-t:execute(
          name=$(echo {1} | sed "s/(.*//; s/ .*//" )
          tldr "$name" 2>/dev/null | bat --style=plain --color=always --language=markdown 2>/dev/null \
            || tealdeer "$name" 2>/dev/null \
            || echo "tldr not installed (yay -S tealdeer)"
        )'
  )
